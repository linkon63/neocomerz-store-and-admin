...
function InventoryReport() {
  ...
  const inStock = products.filter((p) => (p.variants?.[0]?.stockQuantity ?? 0) > 5).length;
  ...
  return (
    <div className="space-y-6">
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <MetricCard label="Inventory Value" value={masked ? "৳***************" : formatCurrency(totalValue)} sub="Total stock value" />
        <MetricCard label="In Stock" value={inStock.toString()} sub="Products with stock > 5" />
        <MetricCard label="Low Stock" value={lowStock.toString()} sub="Stock between 1–5" />
        <MetricCard label="Out of Stock" value={outOfStock.toString()} sub="Zero stock" />
      </div>
      ...
      <div className="rounded-xl border border-slate-200 bg-white">
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[700px] text-left">
            <thead className="bg-slate-50/60">
              <tr>
                {["Product", "SKU", "Stock", "Price", "Value", "Status"].map((h) => (
                  <th key={h} className="px-6 py-3 text-xs font-semibold uppercase tracking-wide text-slate-600">{h}</th>
                ))}
              </tr>
            </thead>
              ...
              {filtered.length === 0 ? (
                ...
                return (
                  <tr key={p.id} className="hover:bg-slate-50/50">
                    ...
                    <td className="px-6 py-3.5 text-xs text-slate-400">{p.sku ?? "—"}</td>
                    <td className="px-6 py-3.5 text-sm text-slate-600">{qty}</td>
                    <td className="px-6 py-3.5 text-sm text-slate-600">{masked ? "৳***" : formatCurrency(price)}</td>
                    <td className="px-6 py-3.5 text-sm font-medium text-slate-800">{masked ? "৳***" : formatCurrency(price * qty)}</td>
                    <td className="px-6 py-3.5"><StatusBadge status={stockStatus} /></td>
                  </tr>
                );
              })}
            </tbody>
