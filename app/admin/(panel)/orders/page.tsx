                ...
                orders.map((order) => (
                  ...
                  >
                    <div className="min-w-0">
                      ...
                      <p className="font-medium text-slate-600">{formatDateTime(order.placedAt)}</p>
                      <span className={`mt-2 inline-flex rounded-md border px-2 py-1 text-xs font-black capitalize ${statusTone(order.status)}`}>
                        {order.status}
                      </span>
                    </div>
                    <div className="text-right">
                      ...
                      <span className={`mt-2 inline-flex rounded-md border px-2 py-1 text-xs font-black capitalize ${paymentTone(order.paymentStatus)}`}>
                        {order.paymentStatus}
                      </span>
                    </div>
                  </button>
              ...
              <>
                <div className="mb-7 flex flex-col justify-between gap-4 rounded-xl bg-white p-4 sm:flex-row sm:items-center">
                  <div className="flex items-center gap-3">
                    <span className={`grid h-12 w-12 place-items-center rounded-full ${statusTone(selected.status)}`}>
                ...
                <article className="rounded-xl border border-slate-200 bg-white p-7">
                  <div className="grid gap-6 border-b border-slate-200 pb-7 lg:grid-cols-[1fr_360px]">
                    ...
                    <div className="grid grid-cols-[1fr_1fr] gap-y-3 text-right font-medium">
                      ...
                      <p>
                        <span className={`rounded-md border px-2 py-1 font-black capitalize ${paymentTone(selected.paymentStatus)}`}>
                    ...
                    <div className="space-y-5">
                      {(selected.items ?? []).map((item, index) => (
                        <div className="flex gap-4" key={item.id}>
                          <ProductThumb color={["bg-blue-600", "bg-emerald-700", "bg-violet-600"][index % 3]} />
                          <div className="min-w-0">
                    ...
                    {totals && (
                      <div>
                        <div className="grid grid-cols-2 gap-y-3 text-right font-medium">
                          ...
                          <p className="border-t border-slate-200 pt-3 text-left font-black">Total</p>
                          <p className="border-t border-slate-200 pt-3 font-black">{formatMoney(totals.grand)}</p>
                          <p className="text-left font-black text-emerald-600">Paid Amount</p>
                          <p className="font-black text-emerald-600">{formatMoney(totals.paid)}</p>
                          <p className="text-left font-black text-rose-600">Due Amount</p>
                          <p className="font-black text-rose-600">{formatMoney(totals.due)}</p>
                        </div>
                      </div>
                    )}
                  </div>
