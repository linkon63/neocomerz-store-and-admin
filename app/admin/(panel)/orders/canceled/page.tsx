...
export default function CanceledOrdersPage() {
  ...
  const loadOrders = useCallback(async () => {
    ...
    try {
      ...
      const canceled = data.filter((o) => o.status === "cancelled" || o.status === "returned");
      setOrders(canceled);
      if (canceled.length > 0 && !selectedId) setSelectedId(canceled[0].id);
    } catch (err) {
  ...
  const handleRestore = useCallback(async () => {
    ...
    try {
      await apiRequest(`/orders/${selected.id}/status`, {
        ...
        body: JSON.stringify({ status: "pending", note: "Order restored from cancellation" }),
      });
  ...
  return (
    <>
      <PageHeader
        ...
        description="Canceled orders content will be here"
        action={
          <div className="flex gap-3">
            <Button
              ...
              onClick={() => {
                ...
                const a = Object.assign(document.createElement("a"), { href: URL.createObjectURL(new Blob([csv],{type:"text/csv"})), download: `canceled-orders-${new Date().toISOString().slice(0,10)}.csv` });
                a.click();
              }}
            >
              Export CSV
      ...
      <div className="mb-8 grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <StatsCard
          ...
          icon={<AdminIcon className="h-6 w-6" name="refresh" />}
        />
        <StatsCard
          label="Total Value"
            ...
            {isLoading ? (
              <div className="flex h-full items-center justify-center py-24">
                <div className="text-center">
                  ...
                  <p className={`${Typography.body} text-gray-500`}>Loading canceled orders...</p>
                </div>
              </div>
            ) : paginatedOrders.length === 0 ? (
              <div className="flex h-full flex-col items-center justify-center py-24">
                <EmptyState
                  icon={<AdminIcon className="h-8 w-8 text-gray-400" name="x" />}
                  title="No Canceled Orders Found"
                  description="No canceled or returned orders match your search criteria."
                />
              </div>
            ) : (
              <>
                  ...
                  {paginatedOrders.map((order) => (
                    <button
                      ...
                      className={`flex w-full items-start justify-between gap-4 px-4 py-5 text-left transition-all duration-200 hover:bg-slate-50 ${
                        selectedId === order.id 
                          ? "bg-red-50 border-r-[3px] border-r-red-600" 
                          : ""
                      }`}
                    >
                ...
                {totalPages > 1 && (
                  <div className="sticky bottom-0 flex items-center justify-between border-t border-gray-200 bg-white px-4 py-3">
                    <p className={`${Typography.body} text-gray-600`}>
                      Page <span className="font-bold text-red-600">{page}</span> of <span className="font-bold text-red-600">{totalPages}</span>
      ...
      <ConfirmModal
        ...
        title="Restore Order"
        message={`Are you sure you want to restore order ${selected?.orderNumber}? It will be moved back to Pending status.`}
        confirmText={isActioning ? "Restoring\u2026" : "Yes, Restore Order"}
      />
    </>
  );
}
